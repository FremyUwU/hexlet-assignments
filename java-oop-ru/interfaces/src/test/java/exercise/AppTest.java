...
class AppTest {
    ...
    void testReversedSequence(){
        CharSequence text = new ReversedSequence("text");

        assertThat(text.toString()).isEqualTo("txet");

        assertThat(text.length()).isEqualTo(4);

        assertThat(text.charAt(1)).isEqualTo('x');
        assertThat(text.subSequence(1, 3)).isEqualTo("xe");
    }
    // END
}
